...
| `contact.html` | 문의 · 자료 요청 |

`img-` 로 시작하는 파일은 사진입니다. 자료 PDF는 페이지에서 내려받지 않고 메일 요청으로만 발송합니다(`sdhaus-*.pdf` 는 업로드하지 않아도 됩니다). `site.css`와 `site.js`는 반드시 함께 올라가야 합니다.

## 아직 확인이 필요한 것

- 연혁 2023년 항목과 사업자 정보
- 시공사진 설명 (사진을 보고 추정해 작성했습니다)
- 자료는 전부 메일 요청 방식입니다 (2026-08 변경)
